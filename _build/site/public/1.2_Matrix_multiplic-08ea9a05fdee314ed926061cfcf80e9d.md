...
Using the rule given in Definition (matrix-multiplication-definition), to calculate $[AB]_{11}$ (the $(1,1)$th entry in the product $AB$) we use row $i=1$ and column $j=1$. We move through the elements from row 1 of $A$ and column 1 of $B$, multiplying the corresponding values and summing the result:
