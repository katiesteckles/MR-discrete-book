...
Note that since $\mathbf{n}$ is non-zero, $\ell_1$ and $\ell_2$ are skew lines. Using {prf-ref}`the equation for the distance between two lines<line-line-distance-theorem>`:
