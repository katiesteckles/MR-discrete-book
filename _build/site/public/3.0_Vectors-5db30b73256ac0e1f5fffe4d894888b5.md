...
An $n$-dimensional Euclidean space is defined by $n$ perpendicular real axes, and is referred to as $\mathbb{R}^n$. For example, consider the diagram of $\mathbb{R}^3$ shown above. Here we can see a portion of 3-dimensional Euclidean space defined by the 3 axes labelled $x$, $y$ and $z$.
...
:width: 400
:alt: Photograph of a person's right hand, with the thumb, index and middle fingers lining up with a set of $x$, $y$ and $z$ axes overlaid
...
Consider the vector $\mathbf{a}$ in $\mathbb{R}^3$ as shown below. Here the displacement along the $x$, $y$ and $z$ axes are $a_1$, $a_2$ and $a_3$ respectively, therefore $\mathbf{a} = (a_1, a_2, a_3)$.

```{figure} /_images/3_R3_vector.svg
:name: R3-vector-figure
:width: 500
:alt: Diagram of a cuboid with an arrow pointing from one corner to the diagonally opposite corner. The sides of the cuboid are labelled $a_1$, $a_2$ and $a_3$ and the arrow is labelled a. There is a set of 3D axes nearby
:class: dark:hidden

The vector $\mathbf{a} = (a_1, a_2, a_3)$ in $\mathbb{R}^3$.
```

```{figure} /_images/3_R3_vector-w.svg
:name: R3-vector-figure-w
:width: 500
:alt: Diagram of a cuboid with an arrow pointing from one corner to the diagonally opposite corner. The sides of the cuboid are labelled $a_1$, $a_2$ and $a_3$ and the arrow is labelled a. There is a set of 3D axes nearby
:class: hidden dark:block
...
A **position vector** $\mathbf{p}$ is a vector that points from the origin $(0, 0, 0)$ to a point in the space.

```{figure} /_images/3_position_vector.svg
:name: position-vector-figure
:width: 350
:alt: An arrow pointing from the labelled origin point $(0,0,0)$ of a set of 3D axes to a point labelled $(x,y,z)$. The arrow is labelled p.
:class: dark:hidden

The position vector $\mathbf{p}$ points from the origin $O$ to a point in the space.
```

```{figure} /_images/3_position_vector-w.svg
:name: position-vector-figure-w
:width: 350
:alt: An arrow pointing from the labelled origin point $(0,0,0)$ of a set of 3D axes to a point labelled $(x,y,z)$. The arrow is labelled p.
:class: hidden dark:block
