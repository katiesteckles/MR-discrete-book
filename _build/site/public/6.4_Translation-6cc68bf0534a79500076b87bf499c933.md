...
Luckily, this can be achieved by appending the third row and column of the identity matrix to $Rot(\theta)$, $Re\!f(\theta)$ and $S(\mathbf{s})$ from the {prf:ref}`rotation<rotation-in-R2-theorem>`, {prf:ref}`reflection<reflection-theorem>` and {prf:ref}`scaling<scaling-theorem>` equations, i.e.,
