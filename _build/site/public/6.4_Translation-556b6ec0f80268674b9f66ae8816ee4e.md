...
```{admonition} Definition 6.9 - Translation
...
```
```{figure} /_images/6_translation-w.svg
:name: translation-figure-w
:width: 400
...
```{admonition} Definition 6.10 - Homogeneous co-ordinates
...
```{admonition} Theorem 6.6 - Translation in $\mathbb{R}^2$
...
```
```{figure} ../_images/6_translation_example-w.svg
:width: 400
:class: hidden dark:block
...
Luckily, this can be achieved by appending the third row and column of the identity matrix to $Rot(\theta)$, $Re\!f(\theta)$ and $S(\mathbf{s})$ from the definitions of {prf:ref}`rotation<rotation-in-R2-theorem>`, {prf:ref}`reflection<reflection-theorem>` and {prf:ref}`scaling<scaling-theorem>`, i.e.,
...
```{figure} ../_images/6_rotation_scaling_and_translation_example_1-w.svg
:width: 400
:class: hidden dark:block
Triangle scaled up by $\mathbf{s} = (2, 2)$
```
...
```
```{figure} /_images/6_rotation_scaling_and_translation_example_2-w.svg
:width: 400
:class: hidden dark:block
The scaled triangle rotated by angle $\theta=\frac{\pi}{4}$ anti-clockwise.
```
...
```{figure} /_images/6_rotation_scaling_and_translation_example_3-w.svg
:width: 700
:class: hidden dark:block
:align: center
