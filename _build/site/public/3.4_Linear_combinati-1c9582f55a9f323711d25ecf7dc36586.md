...
```{admonition} Definition 3.8 - Linear combination of vectors
...
A **basis vector** is a special type of vector, and a set of basis vectors form a **basis** for a vector space. All other vectors in the space can be represented as a linear combination of the basis vectors (we will cover basis in more detail [later](#basis-section)). In Cartesian space, the simplest basis vectors are unit vectors that point in the coordinate directions. In $\mathbb{R}^3$ we use the basis vectors $\mathbf{i} = (1, 0, 0)$, $\mathbf{j} = (0, 1, 0)$ and $\mathbf{k} = (0, 0, 1)$.

```{figure} ../_images/3_basis_vectors.svg
:name: basis-vectors-figure
:width: 225
:alt: A set of 3D coordinate axes is labelled $x$, $y$ and $z$. Three vectors, each starting from the origin, lie incident with each of the axes, labelled $i$, $j$ and $k$ respectively, and go the same distance along each axis
:class: dark:hidden

The three basis vectors in $\mathbb{R}^3$.
```

```{figure} ../_images/3_basis_vectors-w.svg
:name: basis-vectors-figure-w
:width: 225
:alt: A set of 3D coordinate axes is labelled $x$, $y$ and $z$. Three vectors, each starting from the origin, lie incident with each of the axes, labelled $i$, $j$ and $k$ respectively, and go the same distance along each axis
