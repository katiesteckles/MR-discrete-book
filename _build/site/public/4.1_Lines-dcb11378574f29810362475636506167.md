...
```{admonition} Definition 4.1 - Vector equation of a line
...
In other words, every point on $\ell$ is obtained by adding some scalar multiple of the vector $\mathbf{d}$ to the point $\mathbf{p}$. Alternatively, $\ell$ is the line which passes through $\mathbf{p}$ in the direction of $\mathbf{d}$. We call the vector $\mathbf{d}$ a **direction vector** of the line.

```{figure} ../_images/4_vector_equation_of_a_line.svg
:name: line-vector-equation-figure
:width: 500
:alt: A line in 3D space, labelled $\ell$, has a series of points on it: p-d, p, p+d and p+1.5d. The section of the line from p to p+d is a vector labelled d.
:class: dark:hidden

The position of any point on the line $\ell$ can be obtained by adding a scalar multiple of $\mathbf{d}$ to $\mathbf{p}$.
```

```{figure} ../_images/4_vector_equation_of_a_line-w.svg
:name: line-vector-equation-figure-w
:width: 500
:alt: A line in 3D space, labelled $\ell$, has a series of points on it: p-d, p, p+d and p+1.5d. The section of the line from p to p+d is a vector labelled d.
...
The equation of the line that passes through the two points with position vectors $\mathbf{p}_1$ and $\mathbf{p}_2$ can be determined by using {prf:ref}`the vector equation of a line<vector-equation-of-a-line-definition>` with $\mathbf{p} = \mathbf{p}_1$ and $\mathbf{d} = \mathbf{p}_2 - \mathbf{p}_1$. In particular, $\mathbf{d}$ and $\mathbf{p}$ are not unique, and we can choose any point on the line for $\mathbf{p}$, and any non-zero scalar multiple of $\mathbf{d}$, to express the same line.
...
````{admonition} Definition 4.2 - Intersection between two lines
...
:width: 500
:alt: Two lines in 3D space, labelled $\ell_1$ and $\ell_2$, are shown intersecting at a point labelled 'intersection'. The points $p_1$ and $p_2$ lie one on each line, and the vectors $d_1$ and $d_2$ each run from their respective $p_i$ and lie along the line.
:class: dark:hidden

The intersection between the two lines $\ell_1$ and $\ell_2$.
```
```{figure} /_images/4_line_line_intersection-w.svg
:name: line-line-intersection-figure-w
:width: 500
:class: hidden dark:block
...
````{admonition} Definition 4.3 - Parallel lines
:label: parallel-lines-definition
 
Two lines $\ell_1$ and $\ell_2$ in $\mathbb{R}^n$ defined by the vector equations $\mathbf{r}_1 = \mathbf{p}_1 + t_1 \mathbf{d}_1$ and $\mathbf{r}_2 = \mathbf{p}_2 + t_2 \mathbf{d}_2$ respectively are said to be **parallel** if their direction vectors are parallel; that is, $\mathbf{d_1} = k\mathbf{d_2},$ for some non-zero scalar $k$.

```{figure} ../_images/4_parallel_lines.svg
:name: parallel-lines-figure
:width: 500
:alt: Two lines in 3D space, labelled $\ell_1$ and $\ell_2$, are shown lying parallel to each other. The vectors $d_1$ and $d_2$ each lie along their respective lines, and $d_2$ is labelled $\mathbf{d_2}=k\mathbf{d_1}$ and is longer than $d_1$
:class: dark:hidden

The lines $\ell_1$ and $\ell_2$ are parallel in $\mathbb{R}^3$.
```

```{figure} ../_images/4_parallel_lines-w.svg
:name: parallel-lines-figure-w
:width: 500
:alt: Two lines in 3D space, labelled $\ell_1$ and $\ell_2$, are shown lying parallel to each other. The vectors $d_1$ and $d_2$ each lie along their respective lines, and $d_2$ is labelled $\mathbf{d_2}=k\mathbf{d_1}$ and is longer than $d_1$
:class: hidden dark:block
...
````{admonition} Definition 4.4 - Skew lines
:label: skew-lines-definition

Two distinct lines in $\mathbb{R}^3$, which neither intersect nor are parallel, are called **skew lines**.

```{figure} ../_images/4_skew_lines.svg
:name: skew-lines-figure
:width: 350
:alt: Two square sections of plane are shown in 3D space. The planes are parallel to each other, and each contains a line; the two lines point in different directions
:class: dark:hidden

The lines $\ell_1$ and $\ell_2$ are skew lines in $\mathbb{R}^3$.
```

```{figure} ../_images/4_skew_lines-w.svg
:name: skew-lines-figure-w
:width: 350
:alt: Two square sections of plane are shown in 3D space. The planes are parallel to each other, and each contains a line; the two lines point in different directions
:class: hidden dark:block
...
```{admonition} Definition 4.5 - Perpendicular lines
