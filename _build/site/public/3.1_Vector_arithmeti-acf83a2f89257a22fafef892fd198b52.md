...
```{admonition} Definition 3.1 - Vector equality
...
Consider the image below, which shows four vectors in $\mathbb{R}^2$. The vectors $\mathbf{a}$ and $\mathbf{b}$ point in the same direction and have the same magnitude, so we can say that $\mathbf{a}=\mathbf{b}$. The vector $\mathbf{c}$ has the same magnitude as $\mathbf{a}$ and $\mathbf{b}$, but points in the opposite direction, so $\mathbf{c}\neq \mathbf{a}$. The vector $\mathbf{d}$ points in the same direction as $\mathbf{a}$, but has a larger magnitude - so $\mathbf{d} \neq \mathbf{a}$.

```{figure} /_images/3_vector_equality.svg
:name: vector-equality-figure
:width: 450
:alt: A pair of 2D axes, with four labelled arrows. The arrow labelled a points diagonally up and to the right; b is the same arrow shifted up and right a bit; c points in the opposite direction, and d points in the same direction as a but is longer
:class: dark:hidden

Of these four vectors only $\mathbf{a}$ and $\mathbf{b}$ are equal.
```

```{figure} /_images/3_vector_equality-w.svg
:name: vector-equality-figure-w
:width: 450
:alt: A pair of 2D axes, with four labelled arrows. The arrow labelled a points diagonally up and to the right; b is the same arrow shifted up and right a bit; c points in the opposite direction, and d points in the same direction as a but is longer
...
```{admonition} Definition 3.2 - Addition of two vectors
...
A geometric representation of the sum $\mathbf{a}+\mathbf{b}$ is shown below. We place the vector $\mathbf{a}$, and then position the vector $\mathbf{b}$ so its tail is at the head of $\mathbf{a}$. The endpoint of this instance of $\mathbf{b}$ is the head of the vector $\mathbf{a+b}$.
...
:width: 500
:alt: Set of 2D axes showing the vectors a and b, which point in different directions; a copy of b is attached at the end of a, and a copy of a is attached at the end of b; both pairs start and end in the same place. There is a vector running from the start to the end labelled a+b, and on each side the lengths in that direction (and the length of the sum) are labelled
:class: dark:hidden

The addition of two vectors.
```

```{figure} ../_images/3_vector_addition-w.svg
:name: vector-addition-figure-w
:width: 500
:alt: Set of 2D axes showing the vectors a and b, which point in different directions; a copy of b is attached at the end of a, and a copy of a is attached at the end of b; both pairs start and end in the same place. There is a vector running from the start to the end labelled a+b, and on each side the lengths in that direction (and the length of the sum) are labelled
:class: hidden dark:block
...
```{admonition} Definition 3.3 - Scalar multiplication of a vector
...
```{admonition} Theorem 3.1 - Properties of scalar multiplication of vectors
...
:width: 500
:alt: A set of five arrows, labelled a, -a, 1/2a, 2a and -1/2a, with corresponding arrows
:class: dark:hidden

Scaled variations of the vector $\mathbf{a}$.
```

```{figure} /_images/3_scalar_multiplication_of_a_vector-w.svg
:name: scalar-multiplication-of-a-vector-figure-w
:width: 500
:alt: A set of five arrows, labelled a, -a, 1/2a, 2a and -1/2a, with corresponding arrows
:class: hidden dark:block
