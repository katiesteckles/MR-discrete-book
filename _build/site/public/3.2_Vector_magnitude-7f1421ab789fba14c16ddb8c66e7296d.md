...
The **magnitude** of a vector $\mathbf{a}$ is the straight-line distance between the head and tail of $\mathbf{a}$. In two dimensions, this can be calculated using **Pythagoras' Theorem**, and in higher dimensions we can use an extended version of Pythagoras' theorem.

````{admonition} Definition 3.4 - Vector magnitude
:class: note
...
:name: magnitude
:width: 250
:alt: A blue arrow is labelled a. The length of the arrow is labelled ||a||.
:class: dark:hidden
```

```{figure} ../_images/3_magnitude-w.svg
:name: magnitude
:width: 250
:alt: A blue arrow is labelled a. The length of the arrow is labelled ||a||.
...
```{admonition} Theorem 3.2 - Properties of vector magnitude
...
```{admonition} Definition 3.5 - Unit vectors
...
```{admonition} Theorem 3.3 - Normalising a vector
