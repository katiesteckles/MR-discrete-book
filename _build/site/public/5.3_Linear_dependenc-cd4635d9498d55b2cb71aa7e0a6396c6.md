...
```{admonition} Definition 5.4 - Linear dependence
...
So if any two members of a set are scalar multiples of each other, then the set is linearly dependent, because we can choose $\alpha_i$ values to satisfy {prf:ref}`linear dependence<linear-dependence-definition>`.
...
:label: linear-dependence-example
:class: hint
...
(i) &emsp; Let $\alpha_1, \alpha_2, \alpha_3 \in \mathbb{R}$. Then {prf:ref}`the equation for linear dependence<linear-dependence-definition>` becomes
