...
We can use the standard basis $\mathbf{e}_1, \mathbf{e}_2, \ldots \mathbf{e}_n$, which consists of vectors of the form $(1,0,0,...), (0,1,0,...),$ etc.

```{admonition} Definition 6.3 - Transformation matrix
...
The effect of applying the linear transformation from {prf:ref}`linear-transformation-example` is illustrated below. Note that the transformation $T$ can be thought of as changing the basis of the vector space. The unit square with respect to the basis $\{\mathbf{e}_1, \mathbf{e}_2\}$ has been transformed into a unit parallelogram with respect to the basis $\{ T(\mathbf{e}_1), T(\mathbf{e}_2)\}$.
...
:name: linear-transformation-figure-w
:width: 400
...
Therefore we can multiply by the inverse of the matrix of vectors $\mathbf{u}_i$ on the right to get an expression we can use to work out $A$:
...
```{admonition} Theorem 6.1 - Determining the linear transformation given the inputs and image vectors
...
```{admonition} Definition 6.4 - Inverse linear transformation
:label: inverse-transformation-definition

Let $T: V \to W$ be a linear transformation with the transformation matrix $A$. Then $T$ has an **inverse transformation** denoted by $T^{-1}: W \to V$ which reverses the effects of $T$. If $\mathbf{v} \in V$, and $\mathbf{w}$ is the image of $\mathbf{v}$ in $W$, then:
