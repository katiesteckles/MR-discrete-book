...
In $\mathbb{R}^n$, the shortest distance $d$ between a line $\ell$ consisting of points of the form $\mathbf{r} = \mathbf{p} + t\mathbf{d}$, and a point $\mathbf{q}$ off the line, is the length of the vector $\mathbf{q} - \mathbf{r}$, which is perpendicular to $\mathbf{d}$.

```{figure} ../_images/4_line_point_distance.svg
:name: line-point-distance-figure
:width: 425
:alt: a line labelled $\ell$ contains the points p and $r = p+td$; the vector d runs from p in the direction of the line, and there is also a vector at r, running perpendicular to $\ell$ to a point q off the line
:class: dark:hidden
...
```
```{figure} ../_images/4_line_point_distance-w.svg
:name: line-point-distance-figure-w
:width: 425
:alt: a line labelled $\ell$ contains the points p and $r = p+td$; the vector d runs from p in the direction of the line, and there is also a vector at r, running perpendicular to $\ell$ to a point q off the line
...
```{admonition} Theorem 4.1 - The shortest distance between a point and a line
...
so the direction vector is $\mathbf{d} = (1, 1, 1)$. Using {prf:ref}`the equation for the distance from a point to a line<point-line-distance-theorem>`:
...
- If the two lines are skew, then the shortest distance is the length of the chord between them - this will be a vector that is perpendicular to both $\ell_1$ and $\ell_2$.

```{figure} ../_images/4_line_line_distance.svg
:name: line-line-distance-figure
:width: 350
:alt: Two planes in 3D space lie, one above the other, each containing a point r and a line $\ell$, defined by a point p and direction vector d. The normal vector $\hat{n}$ is perpendicular to the lower line at the point r, and points towards the point r on the line above.
:class: dark:hidden
...
:name: line-line-distance-figure-w
:width: 350
:alt: Two planes in 3D space lie, one above the other, each containing a point r and a line $\ell$, defined by a point p and direction vector d. The normal vector $\hat{n}$ is perpendicular to the lower line at the point r, and points towards the point r on the line above.
:class: hidden dark:block
...
The term in brackets here is the direction vector of the chord, which will be a normal vector to both planes. We can write $\hat{\mathbf{n}} = \dfrac{\mathbf{d}_1 \times \mathbf{d}_2}{\|\mathbf{d}_1 \times \mathbf{d}_2\|}$. Now we substitute into this the equations of $\mathbf{r}_1$ and $\mathbf{r}_2$, to give:
...
```{admonition} Theorem 4.2 - The shortest distance between two skew lines
...
Note that since $\mathbf{n}$ is non-zero, $\ell_1$ and $\ell_2$ are skew lines. Using the {prf:ref}`equation for the distance between two lines<line-line-distance-theorem>`:
