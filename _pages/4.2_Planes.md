...
````{admonition} Definition 4.6 - Vector equation of a plane
...
where $\mathbf{r}$ is a point on the plane, and $t_1,t_2 \in \mathbb{R}$ are parameters. 

```{figure} /_images/4_plane.svg
:name: plane-1-figure
:width: 400
:alt: A square section of plane lies horizontally, and contains two points labelled $p$ and $r$. The vector $t_1\mathbf{d_1}$ runs from $p$ to the start of the vector $t_2\mathbf{d_2}$, which runs from there to the point $r$.
:class: dark:hidden
...
```
```{figure} /_images/4_plane-w.svg
:name: plane-1-figure-w
:width: 400
:alt: A square section of plane lies horizontally, and contains two points labelled $p$ and $r$. The vector $t_1\mathbf{d_1}$ runs from $p$ to the start of the vector $t_2\mathbf{d_2}$, which runs from there to the point $r$.
...
````{admonition} Definition 4.7 - The normal vector
:class: note
...
:name: plane-2-figure
:width: 450
:alt: A square section of plane contains two vectors a and b, which start in the same place, and their cross product $n=a\times b$ which extends vertically, at right angles to a and b
:class: dark:hidden

The normal vector $\mathbf{n}$ is perpendicular to the plane that the vectors $\mathbf{a}$ and $\mathbf{b}$ lie on.
```
```{figure} ../_images/4_normal_vector-w.svg
:name: plane-2-figure-w
:width: 450
:alt: A square section of plane contains two vectors a and b, which start in the same place, and their cross product $n=a\times b$ which extends vertically, at right angles to a and b
:class: hidden dark:block
...
(since the normal vector will be perpendicular to any vector going between two points in the plane it is normal to). We can also write:
...
````{admonition} Definition 4.8 - Point-normal form of a plane
:class: note
...
:width: 450
:alt: A square section of plane contains two points p and r; a vector runs from p to r in the plane, and another vector labelled n extends vertically upwards from the point p, at right angles to the first vector
:class: dark:hidden

A plane can be defined by a point on the plane and the normal vector to the plane.
```
```{figure} ../_images/4_point_normal-w.svg
:name: point-normal-figure-w
:width: 450
:alt: A square section of plane contains two points p and r; a vector runs from p to r in the plane, and another vector labelled n extends vertically upwards from the point p, at right angles to the first vector
:class: hidden dark:block
...
Using {prf:ref}`the point-normal form equation<point-normal-definition>` with $\mathbf{p} = \mathbf{p}_1$
